    ...
    async excluiCurso (req, res){
        try {
         ...
         res.status(204).json({})
            ...
            console.log(error.message)
            res.status(500).json({ error: 'Não possível excluir o curso' })           
        }
        
    }

    //Alterar um Curso
    async alteraCurso (req, res){
        try {
            const { id } = req.params
            ...
            console.log(error.message)
            res.status(500).json({ error: 'Não possível Alterar o curso' })             
        }
